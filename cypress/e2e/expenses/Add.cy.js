...
describe('Add expense - not within current month', () => {
  ...
  it('checks that the form has been cleared', () => {
    cy.findByRole('textbox', { name: /expense name/i }).should('have.value', '');
    cy.findByLabelText(/expense date/i).should('have.value', '');
    cy.findByRole('spinbutton', { name: /expense amount/i }).should('have.value', '');
  });

  it('checks that new expense is correctly rendered', () => {
    cy.get('[data-cy=expenses]').should('have.length', 1).and('not.contain', 'You have no prior expenses');
    cy.fixture('expense-post').then(expense => {
      cy.get('[data-cy="expenses"] > li').should('contain', '01/03/2022').and('contain', 'Test Expense - Cypress').and('contain', '- €4.99');
    });
  });
  ...
  it("checks that the new expense is not rendered in other month's history", () => {
    cy.findByRole('combobox', { name: /select a month/i }).select(CURRENT_MONTH);
    cy.get('[data-cy="expenses"]').should('not.contain', '01/03/2022').and('not.contain', 'Test Expense - Cypress').and('not.contain', '- €4.99');
  });
});
