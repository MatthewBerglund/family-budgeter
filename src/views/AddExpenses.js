...
const AddExpenses = ({ addExpense, setNewExpenseMonth }) => {
  ...
  const handleDateChange = date => {
    ...
    setNewExpenseMonth(getUKFormattedDate(date, { year: 'numeric', month: 'long' }));
  };
      ...
      <div className="card-body">
        <form className="row g-2" onSubmit={handleSubmit}>
          <div className={`col-lg-6 ${wasTitleValidated ? 'was-validated' : ''}`}>
            ...
            <input className="form-control" type="text" id="name" name="name" placeholder="e.g. Groceries" aria-describedby="nameHelp" required autoFocus value={title} onChange={handleTitleChange} />
          </div>
          <div
          ...
          >
            ...
            <DatePicker className="form-control" id="date" name="date" placeholderText="DD/MM/YYYY" aria-describedby="dateHelp" required selected={date} onChange={handleDateChange} dateFormat="dd/MM/yyyy" todayButton="Today" />
          </div>
          <div className={`col mb-2 ${wasAmountValidated ? 'was-validated' : ''}`}>
            ...
            <div className="input-group">
              ...
              <input className="form-control" type="number" id="amount" name="amount" placeholder="e.g. 19.99" aria-describedby="amountHelp" required min="0.01" step="0.01" value={amount} onChange={handleAmountChange} />
            </div>
          </div>
